...
use crate::types::{CsvInspection, QueryResult, TablePreview};
...
impl CsvAdapter {
    pub fn new(path: &str, config_json: &str) -> Result<Self, String> {
        let config: CsvConfig =
            serde_json::from_str(config_json).map_err(|e| format!("Invalid CSV config: {}", e))?;
...
pub fn inspect_csv(path_str: &str) -> Result<CsvInspection, String> {
    ...
    if path.is_dir() {
        return Err(format!(
            "Selected path is a directory, not a CSV file: {}.",
            path_str
        ));
    }
    ...
    let extension = path
        ...
        .unwrap_or("")
        .to_lowercase();
    let separator = if extension == "tsv" || extension == "tab" {
        '\t'
    } else {
        ','
    };
    ...
    for i in 0..columns.len() {
        ...
        for row in &samples {
            ...
            if val.is_empty() {
                continue;
            } // Empty can be anything (likely NULL)
...
impl DatabaseAdapter for CsvAdapter {
    async fn connect(&mut self) -> Result<(), String> {
        // 1. Create Temp File
        let temp_file =
            NamedTempFile::new().map_err(|e| format!("Failed to create temp database: {}", e))?;
        ...
        for (i, (col, date_type)) in self
            .config
            .columns
            .iter()
            .zip(self.config.types.iter())
            .enumerate()
        {
        ...
        tokio::task::spawn_blocking(move || {
            let file = match File::open(&path_clone) {
                ...
                Err(e) => {
                    let _ = tx.blocking_send((
                        vec![],
                        vec![(0, "".to_string(), format!("Failed to open file: {}", e))],
                    ));
                    return;
                }
            };
            ...
            for result in rdr.records() {
                            ...
                            for (i, val) in row.iter().enumerate() {
                                ...
                                if expected_type == "INTEGER" {
                                    if val.parse::<i64>().is_err() {
                                        error = Some(format!(
                                            "Type mismatch in col {}: expected INTEGER, got '{}'",
                                            config_clone.columns[i], val
                                        ));
                                        break;
                                    }
                                } else if expected_type == "REAL" && val.parse::<f64>().is_err() {
                                    error = Some(format!(
                                        "Type mismatch in col {}: expected REAL, got '{}'",
                                        config_clone.columns[i], val
                                    ));
                                    break;
                                }
                ...
                if valid_batch.len() >= 1000 || bad_batch.len() >= 1000 {
                    if tx
                        .blocking_send((valid_batch.clone(), bad_batch.clone()))
                        .is_err()
                    {
                        break;
                    }
        ...
        while let Some((valid_batch, bad_batch)) = rx.recv().await {
            ...
            if !bad_batch.is_empty() {
                let mut tx = pool
                    .begin()
                    .await
                    .map_err(|e| format!("Failed to begin transaction for bad rows: {}", e))?;
                for (line, content, reason) in bad_batch {
                    sqlx::query(&format!(
                        "INSERT INTO {} (line_number, raw_content, error_reason) VALUES (?, ?, ?)",
                        bad_rows_table
                    ))
                    .bind(line as i64)
                    .bind(content)
                    ...
                    .execute(&mut *tx)
                    .await
                    .map_err(|e| format!("Insert bad row failed: {}", e))?;
                }
                tx.commit()
                    .await
                    .map_err(|e| format!("Commit bad rows failed: {}", e))?;
            }
        }
...
mod tests {
    ...
    async fn test_csv_loading() {
        ...
        // Query valid
        let res = adapter
            .execute_query("SELECT * FROM csv_data ORDER BY id")
            .await
            .expect("Query failed");
        ...
        // Query bad
        let res = adapter
            .execute_query("SELECT * FROM csv_data_bad_rows")
            .await
            .expect("Query bad failed");
        if let QueryResult::ResultSet(preview) = res {
